...
set /a _tries+=1
if %_tries% gtr 30 (
    echo [Pokemon TCG] Server did not start after 60 seconds. Check the PokemonTCG-Server window.
    pause
    exit /b 1
)
timeout /t 2 /nobreak >nul
powershell -Command "try { Invoke-WebRequest -Uri 'http://localhost:3333' -UseBasicParsing -TimeoutSec 2 | Out-Null; exit 0 } catch { exit 1 }" >nul 2>&1
if errorlevel 1 goto wait

:: --- Open Chrome ---
echo [Pokemon TCG] Ready! Opening browser...
for %%C in (
