// Pokédex number lookup for Japanese-style card sorting (Gen 1–3)
...
// Trainer name prefixes to strip (order matters — longer prefixes first)
const TRAINER_PREFIXES = [
  "Team Aqua's ", "Team Magma's ", "Team Rocket's ",
  "Lt. Surge's ", "Giovanni's ", "Sabrina's ",
  "Blaine's ", "Brock's ", "Erika's ", "Koga's ", "Misty's ",
  "Rocket's ",
];

// Modifier prefixes that don't change the base Pokémon
const MOD_PREFIXES = [
  'Snow-cloud ', 'Shining ', 'Flying ', 'Surfing ', 'Light ', 'Dark ', 'Cool ',
  'Rain ', 'Sunny ',
];

function stripPrefixes(name: string): string {
  for (const p of TRAINER_PREFIXES) {
  ...
  // Normalize "Unown [X]" → "Unown"
  name = name.replace(/^Unown\s*\[.*\]$/, 'Unown');
  // Strip "_____'s " prefix (promo Pikachu)
  name = name.replace(/^_+'\s*s\s+/, '');
  return name;
};

export function pokedexNumber(cardName: string): number {
  const base = stripPrefixes(cardName);
  return DEX[base] ?? 9999;
}

// Release order for WotC sets (earlier = lower index)
